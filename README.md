...
I came across the [Elm Language](https://elm-lang.org) a few years ago and it has forever changed the way I approach software. Elm is multiple things: it is a language, it is a package ecosystem and more fundamentally it is an **architecture**.
...
* Declaring the list of `Message`s that your application understands. For the simple counter application, we can imagine three messages: `IncrementClicked`, `DecrementClicked` and `ResetClicked`
...
> ℹ More advanced use cases will require you to implement your specific `Command` and `Subscription` but this is something we can cover in a later topic.
