...
public sealed class GetRandomNumberCommandTests
{
    ...
    [Fact]
    public async Task GetRandomNumberCommand_ReturnsFromInclusive_WhenToInclusiveIsLowerOrEqual()
    {
        var test = async () =>
        {
            var fromInclusive = GetInt32(0, 100);
            var toExclusive = fromInclusive - GetInt32(0, 10);
            ...
            var msg = Assert.IsType<TestMessage.RandomNumberGenerated>(result);
            Assert.Equal(fromInclusive, msg.randomNumber);
        };
